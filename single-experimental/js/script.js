	...
	create: function( options ) {
		return (function(){
			...
			function setupPositions() {
				...
				switch( config.position ) {
					...
					default:
						...
						//0 50% --> 0 50vw
						contentsTransformOrigin = '0 50vw';
						contentsTransformOpened = 'translateX( '+ config.width +'px ) rotateY( ' + -contentsAngle + 'deg )';
			...
			function setupWrapper() {
				...
				dom.wrapper.style[ Meny.prefix( 'perspective' ) ] = '100vw';
			...
			function open() {
				if( !isOpen ) {
					...
					Meny.addClass( dom.wrapper, 'sidenav-active' );
			...
			function close() {
				if( isOpen ) {
					...
					Meny.removeClass( dom.wrapper, 'sidenav-active' );
